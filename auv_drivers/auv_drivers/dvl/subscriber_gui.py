...
class DVLSubscriberGUI(Node):
    def __init__(self):
        super().__init__('a50_sub')
        self.subscription_raw = self.create_subscription(String, "dvl/json_data", self.callback_raw)
        self.subscription_dvl = self.create_subscription(DVL, "dvl/data", self.callback_dvl)
...
def main(args=None):
    ...
    dvl_subscriber_gui = DVLSubscriberGUI()
    rclpy.spin(dvl_subscriber_gui)
    dvl_subscriber_gui.destroy_node()
    rclpy.shutdown()
